...
The persona settings of the respondent, the model-name, and temprature 
...
#-----------------------------------------------------------------------
def respond_likert_item_ollama(item, instruction, persona="", temperature=None):
...
def respond_likert_questionnaire_ollama(questionnaire, instruction, persona="", temperature=None):
    ''' Use Ollama API to generate a responses to a Likert-scale questionnaire. 
          ...
          - temperature: float, temperature to control the generation.
          - return: a list ofdictionary that contains the generated responses to the questionnaire.
    '''
    results = []
    for item in questionnaire:
        res = respond_likert_item_ollama(item, instruction, persona, temperature)
...
def main():
    ...
    # Generate responses
    results = respond_likert_questionnaire_ollama(items, config['instruction'], config['persona'], config['temperature'])
    # Prepare output
